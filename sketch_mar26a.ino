...
void loop(){
  ...
  if(inputB == 'F' && MS.getDistance() > 50 && MS.getDistance() > 0) {   // F = go forward
        g.setSPEED1(50);
        g.setSPEED2(50);
        g.setForward2();
        g.setForward1();
        ...
        g.setBackward2();
        g.runGear();
    } else if(inputB == 'R' && MS.getDistance() > 50 && MS.getDistance() > 0) { // R = turn right
        g.turnRight();
        g.setSPEED1(80);
        g.setSPEED2(0);
        g.runGear();
    }
    else if(inputB == 'L' && MS.getDistance() > 50 && MS.getDistance() > 0) { // L = turn left
      g.turnLeft();
      g.setSPEED1(0);
      g.setSPEED2(80);
      g.runGear();
    ...
    else if(MS.getDistance() <= 50 && MS.getDistance() > 0) {
      g.setSPEED1(0);
      g.setSPEED2(0);
      g.stopGear();
    }
